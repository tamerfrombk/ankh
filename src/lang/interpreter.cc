...
fk::lang::ExprResult fk::lang::Interpreter::visit(CallExpression *expr)
{
    ...
    // I'm not totally happy with this but this is a solution for constructors
    // When a constructor is generated by the parser, we don't put a return statement
    // in the body. Every other function gets injected with a return statement.
    // We take advantage of this here to implement object generation.
    if (data_declarations_.count(name) > 0) {
        Data *data = data_declarations_[name].get();
...
void fk::lang::Interpreter::declare_function(FunctionDeclaration *decl, EnvironmentPtr env)
{
    ...
    CallablePtr callable = make_callable<Function>(this, decl->clone(), env);
...
void fk::lang::Interpreter::visit(DataDeclaration *stmt)
{
    ...
    if (data_declarations_.count(stmt->name.str) > 0) {
        ::panic("{}:{}, '{}' is already a data declaration", stmt->name.line, stmt->name.col, stmt->name.str);
    }

    EnvironmentPtr env = make_env(current_env_);
    ...
    data_declarations_[stmt->name.str] = make_data(stmt->name.str, env);

    declare_function(static_cast<FunctionDeclaration*>(stmt->ctor.get()), env);
