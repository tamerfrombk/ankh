...
fk::lang::ExprResult fk::lang::Interpreter::visit(CallExpression *expr)
{
    ...
    if (data_declarations_.count(name) > 0) {
        Data<ExprResult>* data = data_declarations_[name].get();
        Constructor<ExprResult, Interpreter> ctor(this, data);

        ctor.invoke(expr->args);

        return make_object<ExprResult>(ctor.env());
    }
...
void fk::lang::Interpreter::visit(DataDeclaration *stmt)
{
    ...
    data_declarations_[stmt->name.str] = make_data<ExprResult>(stmt->name.str, env, members);
