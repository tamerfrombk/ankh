...
ankh::lang::Parser::Parser(const std::vector<Token> &tokens) : tokens_(tokens), cursor_(0) {}
...
ankh::lang::StatementPtr ankh::lang::Parser::parse_for() {
    ...
    StatementPtr mutator = check(ankh::lang::TokenType::LBRACE) ? nullptr : statement();

    StatementPtr body = block();

    return make_statement<ForStatement>(for_token, std::move(init), std::move(condition), std::move(mutator),
                                        std::move(body));
}
...
ankh::lang::ExpressionPtr ankh::lang::Parser::expression() { return parse_or(); }
...
ankh::lang::ExpressionPtr ankh::lang::Parser::comparison() {
    ...
    while (match({ankh::lang::TokenType::LT, ankh::lang::TokenType::LTE, ankh::lang::TokenType::GT,
...
ankh::lang::ExpressionPtr ankh::lang::Parser::primary() {
    ...
    if (match({TokenType::NUMBER, TokenType::ANKH_TRUE, TokenType::ANKH_FALSE, TokenType::NIL})) {
...
const ankh::lang::Token &ankh::lang::Parser::prev() const noexcept { return tokens_[cursor_ - 1]; }

const ankh::lang::Token &ankh::lang::Parser::curr() const noexcept { return tokens_[cursor_]; }
...
bool ankh::lang::Parser::is_eof() const noexcept { return curr().type == ankh::lang::TokenType::ANKH_EOF; }
...
bool ankh::lang::Parser::match(std::initializer_list<ankh::lang::TokenType> types) noexcept {
    return std::any_of(types.begin(), types.end(), [&](TokenType type) { return match(type); });
}
...
bool ankh::lang::Parser::check(std::initializer_list<TokenType> types) const noexcept {
    return std::any_of(types.begin(), types.end(), [&](TokenType type) { return check(type); });
}
...
void ankh::lang::Parser::synchronize_next_statement() noexcept {
    const auto statement_initializer_tokens = {
        TokenType::LBRACE, TokenType::IF, TokenType::WHILE, TokenType::FOR,  TokenType::ANKH_RETURN, TokenType::INC,
        TokenType::DEC,    TokenType::FN, TokenType::LET,   TokenType::DATA, TokenType::BREAK,
    };
