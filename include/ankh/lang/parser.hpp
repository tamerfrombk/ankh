...
template <class ExpectedType, class Ptr> ExpectedType *instance(const Ptr &ptr) noexcept {
    return dynamic_cast<ExpectedType *>(ptr.get());
}

template <class ExpectedType, class Ptr> bool instanceof (const Ptr &ptr) noexcept {
